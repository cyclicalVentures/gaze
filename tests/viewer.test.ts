...
test('the physical screen corners stay fixed under head translation in X, Y, and Z', () => {
...
test('behind-screen and front-screen objects have opposite motion parallax', () => {
...
test('near/far movement changes behind-screen size with the correct ratio', () => {
  const camera = new PerspectiveCamera();
...
test('rejects invalid projection geometry', () => {
...
test('calibration reconstructs a known 3D eye displacement and depth', () => {
...
test('One Euro filter suppresses jitter and resets on a new calibration', () => {
...
test('ASCII PLY with faces loads as a normalized mesh', async () => {
...
for (const littleEndian of [true, false]) test(`binary ${littleEndian ? 'little' : 'big'} endian PLY loads as a point cloud`, async () => {
...
test('invalid, empty, oversized and NaN models fail with recovery messages', async () => {
...
test('the supplied Scaniverse binary USDZ loads mesh geometry and binds its JPEG texture', async () => {
