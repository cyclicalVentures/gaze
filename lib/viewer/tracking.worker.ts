...
self.onmessage = async (event: MessageEvent) => {
  if (event.data.type === 'init') {
    try {
      ...
      try { detector = await FaceLandmarker.createFromOptions(wasm, options); }
      catch { detector = await FaceLandmarker.createFromOptions(wasm, { ...options, baseOptions: { ...options.baseOptions, delegate: 'CPU' } }); }
  ...
  if (event.data.type === 'frame') {
    ...
    try {
      ...
      if (landmarks?.[473]) {
        ...
        if (span > 12 && open > 0.006) observation = { x: (a.x + b.x) * bitmap.width / 2, y: (a.y + b.y) * bitmap.height / 2, span, left: { x: b.x * bitmap.width, y: b.y * bitmap.height }, right: { x: a.x * bitmap.width, y: a.y * bitmap.height }, time: event.data.time };
      }
