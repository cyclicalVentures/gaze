...
export default function Home() {
  ...
  const recenter = () => {
    ...
    if (tracker.current?.calibrate()) { engine.current?.calibrateOrigin(); setNote('Centered. Move gently left, right, closer, and farther to look into the box.'); }
    else setNote('Keep both eyes visible and hold still for a moment, then try centering again.');
  };
    ...
    <div className="workspace">
      ...
      <section ref={viewer} className={`viewer ${focus ? 'is-focused' : ''}`} aria-label="3D model viewer" onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setDragging(false); }} onDrop={e => { e.preventDefault(); setDragging(false); const file = e.dataTransfer.files[0]; if (file) void load(file); }}>
        ...
        <div className="viewport-bottom">
          ...
          <div className="viewport-tools">
            ...
            <button className="icon-button" title={focus ? 'Exit full view' : 'Full view'} aria-label={focus ? 'Exit full view' : 'Full view'} onClick={() => void fullscreen()}>{focus ? <X size={18}/> : <Expand size={18}/>}</button>
          </div>
        </div>
      </section>
      <aside className="inspector" aria-label="Viewer controls">
        ...
        <section className="control-section tracking-section" aria-labelledby="tracking-title">
          ...
          <p className="tracking-instruction">{status === 'starting' ? 'Allow the front camera. The tracker may take a few seconds to load.' : status === 'ready' ? 'Face the center of the 3D window, hold still, then set your eye position.' : status === 'lost' ? 'Bring both eyes back into view. The perspective is held until tracking returns.' : tracking ? 'Move side to side or lean in. Recenter after moving your screen or camera.' : 'Tracks your eye position in 3D to update the perspective as you move.'}</p>
          ...
          {active && status !== 'starting' && <button className="button teal-action full-width" onClick={recenter} disabled={status === 'lost'}><Crosshair size={17}/>{tracking ? 'Recenter eye position' : 'Set eye position'}</button>}
